...
const uint32_t AUTHORIZE_USAGE_PAGE = 0xFFFA0000;
...
std::vector<reverse_mapping_t> reverse_mapping_authz;
...
void set_mapping_from_config() {
    ...
    for (auto const& [hub_port_target, sources] : reverse_mapping_map) {
        ...
        if ((target & 0xFFFF0000) == MACRO_USAGE_PAGE) {
          ...
          } else if ((target & 0xFFFF0000) == AUTHORIZE_USAGE_PAGE) {
            reverse_mapping_authz.push_back(rev_map);
        } else {
...
void process_mapping(bool auto_repeat) {
    ...
    for (auto const& rev_map : reverse_mapping_authz) {
      uint16_t i = rev_map.target & 0xFFFF;
      for (auto const& map_source : rev_map.sources) {
          if (!map_source.sticky) {
              if (map_source.layer_mask & layer_state_mask) {
                  authorized = (map_source.hold ? map_source.tap_hold_state->hold : *map_source.input_state);
              }
          } else {
              // is sticky
              if (
